...
\end{align}
%
A successive approximation to $\valuefunc(t; \cdot, \cdot, \cdot)$ consists in  maintaining local approximations to the global system dynamics at every iteration. % with the local state and controls $\state_r, \control_r, \disturb_r$. 
%
%The max-min over the entire control sequences reduces to a stepwise optimization over single controls, starting from a negative time $T$ and going forward in time with %the \textit{stage cost}
%\begin{align}
...
%
\begin{itemize}
	\item approximate the nonlinear system dynamics (c.f. \eqref{eq:sys_dyn}), starting with the pursuer and evader's local control schedules, $\{ \control_r(t)\}_{t=T}^0$ and $\{ \disturb_r(t) \}_{t=T}^0$, assumed to be available; % (when these are not available, we can run the passive dynamics of the system);
	%
	\item  run the system's passive dynamics with $\{\control_r(t)\}_{t=T}^0, \{ {\disturb_r(t)} \}_{t=0}^T$ to generate  nominal state trajectories $ \{\state_r(t)\}_{t=T}^0$, with neighboring trajectories $\{ \state(t) \}_{t=0}^T$; %deterministic dynamics,
	%
