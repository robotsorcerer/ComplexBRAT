...
We adopt the essentially non-oscillatory Lax-Friedrichs scheme of~\cite{OsherShuENO, Crandall1984Approx} in resolving \eqref{eq:HamiltonianCompletada}. We define the numerical monotone flux, 	$\hat{\hamfunc}^{LF}$, of $\hamfunc(\cdot)$ as 
%
\begin{align}
	\hat{\hamfunc}^{LF}(u^+, u^-, v^+, v^-, \bm{w}^+, \bm{w}^-) = \hamfunc\left(\frac{u^+ +u^-}{2}, \frac{v^+ +v^-}{2}, \frac{\bm{w}^+ +\bm{w}^-}{2}\right) \nonumber
	%
	\\
	%
	- \dfrac{1}{2}\left[ \alpha^x\left(u^+ - u^-\right) + \alpha^y\left(v^+ - v^-\right) + \alpha^z\left(\bm{w}^+ - \bm{w}^-\right)\right]
\end{align}
%
where
%
\begin{align}
	\alpha^x = \max_{\substack{ a \le u \le b \\ c \le v \le d  \\ e \le w \le f}} |\hamfunc_u(u,v,w)|, \, \alpha^y = \max_{\substack{ a \le u \le b \\ c \le v \le d  \\ e \le w \le f}} |\hamfunc_v(u,v,w)|, \, \&, \alpha^z = \max_{\substack{ a \le u \le b \\ c \le v \le d  \\ e \le w \le f}} |\hamfunc_w(u,v,w)|
\end{align}
%
are the so-called dissipation coefficients. Here, the subscripts of $\hamfunc$ partial derivatives w.r.t the subscript variable, and the flux is monotone for $a \le u^\pm \le b, c \le v^\pm \le d, e \le w^\pm \le f$