...
\end{align}
%
%For all purposes in this work, we assume there is a single pursuer. However, our scheme is easily extensible to multiple pursuers. It is clear from \eqref{eq:Hamiltonian} that the worst possible disturbance $w_p$ needs to be calculated in order to simulate the robust cohesion that we are after seeing that the orientation of each agent within a flock is not predetermined but rather averaged among the headings of its nearest neighbors. In addition, the heading of each agent within a flock is simulated against that of the pursuer so that every agent can withstand an attacking predator. To do this, we leverage the \textit{broadcast} and \textit{gather} algorithm of~\cite{BoydADMM}, whereupon we compute the respective Hamiltonian of each agent in a flock independently during the \textit{gather} phase, and then collect the respectively updated Hamiltonian at a node during the \textit{gather} phase. This helps speed up the computation of the control laws using the consistent and monotone explicit marching schemes of ~\cite{CrandallLaxFriedrichs, Crandall1984, CrandallFractional}.
