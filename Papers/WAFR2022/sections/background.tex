\section{Reachability for Systems Verification.}
\label{sec:related}
...
%\end{subequations}
%
for constant $k$ and all $T \le t \le 0$, $\hat{\state}, \, \state \in \reline^n$, $\bm{u}\in \mathcal{U}$ and $\disturb \in \mathcal{V}$.  %The zero sublevel set of $g(\state)$ constitutes the target set~\cite{Mitchell2001} and is given by $\mathcal{L}_0 = \{ \state \in \bar{\Omega} \,|\, g(\state) \le 0 \}$. 
%
%\begin{align}
...
%. Therefore, we can cast a target set as the time-resolved terminal value $\lowervalue(\state, T) = g(\state(T))$ so that given a time bound, and an unsafe set of states, the time-bounded safety verification problem consists in certifying that there is no phase within the target set \eqref{eq:target_set} such that the solution to \eqref{eq:sys_dyn} enters the unsafe set. 
...
%\end{align}
%
is the \textit{target set} in the phase space $\openset \times \mathbb{R}$ for a backward reachability problem~\cite{Mitchell2001}. %The reachable set of states could be for example those that lie along the trajectory of \eqref{eq:HJ_traj} such that we evade the unsafe sets up to a time \eg $t_f$ within a given time bound  $\left[t_0, t_f\right]$. 
