...
Exceptions: the unit matrix is $\bm{\identity}$; and $i,j,k,p$ are indices. % We adopt zero-indexing for matrix operations throughout so that if index $i$ corresponds to size $N$, we shall write $i = 0, 1, \cdots , N-1$. %Lastly, for a tensor with $N$ modes, we denote by $\left[N\right]$ the set $\left\{0,1,\cdots,N-1\right\}$.
