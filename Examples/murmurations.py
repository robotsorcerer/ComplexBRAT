...
parser.add_argument('--flock_payoff', '-sp', action='store_false', default=False,
                    help='Visualize individual payoffs within a flock?')
...
parser.add_argument('--pause_time', '-pz', type=float, default=.3,
                    help='Pause time between successive updates of plots')
...
def main(args):
    ...
    INIT_XYZS = np.array(
        [[neigh_rad * np.cos((i / 6) * np.pi / 4), neigh_rad * np.sin((i / 6) * np.pi / 4), H + i * H_STEP] for i in
         range(num_agents)])
    ...
    if strcmp(args.mode, 'single'):
        ...
        if args.load_brt:
            ...
            if args.resume:
                ...
                savename = join(args.out_dir,
                                rf"murmurations_flock_{args.flock_num:0>2}_{datetime.strftime(datetime.now(), '%m-%d-%y_%H-%M')}.hdf5")
